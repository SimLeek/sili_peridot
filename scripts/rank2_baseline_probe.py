...
from scripts.landmark_checklist import N_EVAL, N_STEPS, SEEDS, run_config

CONFIGS = [
    ("baseline_rank2", {"use_energy": False, "all_zero_init": False, "scale_rank": 2}),
    ("baseline_energy_rank2", {"use_energy": True, "all_zero_init": False, "scale_rank": 2}),
]
...
def main():
    ...
    print(
        f"{'config':<24} {'old_style':>10} {'eval_acc':>9} {'rank1_old_ref':>14} {'delta':>8} {'skip_rate':>10} {'avg_step':>10}"
    )
    for name, mean_old, mean_eval, skip_rate, per_seed_eval, avg_step_time in rows:
        ref = rank1_ref[name]
        delta = mean_old - ref
        print(
            f"{name:<24} {mean_old:>10.4f} {mean_eval:>9.4f} {ref:>14.4f} {delta:>+8.4f} {skip_rate:>9.3%} {avg_step_time * 1000:>8.1f}ms"
        )
