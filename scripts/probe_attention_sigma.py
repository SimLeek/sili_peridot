...
def run_probe(
    ...
    print(
        f"  {label}: {n} queries, {n_correct} correct ({100 * n_correct / max(n, 1):.1f}%), "
        f"peak_vocab={r['peak_stage']['vocab']} final_vocab={r['final_vocab']}",
        flush=True,
    )
    if n > 0:
        print(
            f"  sigma (all):       mean={sigmas_all.mean():.4f} std={sigmas_all.std():.4f} "
            f"median={np.median(sigmas_all):.4f} p10={np.percentile(sigmas_all, 10):.4f} "
            f"p90={np.percentile(sigmas_all, 90):.4f}",
            flush=True,
        )
    if n_correct > 0:
        s_correct = sigmas_all[correct_mask]
        print(
            f"  sigma (correct):   mean={s_correct.mean():.4f} std={s_correct.std():.4f} "
            f"median={np.median(s_correct):.4f}",
            flush=True,
        )
    if n - n_correct > 0:
        s_wrong = sigmas_all[~correct_mask]
        print(
            f"  sigma (incorrect): mean={s_wrong.mean():.4f} std={s_wrong.std():.4f} median={np.median(s_wrong):.4f}",
            flush=True,
        )
    return {"label": label, "records": records, "peak_vocab": r["peak_stage"]["vocab"], "final_vocab": r["final_vocab"]}
