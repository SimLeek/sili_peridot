...
PEAK_RE = re.compile(r"PEAK precision=(\S+) peak_vocab=(\d+) peak_k=(\d+) peak_phase=(\S+)")
FINAL_RE = re.compile(
...
def parse_log(path: str) -> dict:
    ...
    with open(path) as f:
        for line in f:
            ...
            if m:
                peak = {"precision": m.group(1), "vocab": int(m.group(2)), "k": int(m.group(3)), "phase": m.group(4)}
...
def main():
    ...
    print(
        f"{'precision':<10} {'peak_vocab':>10} {'peak_k':>7} {'peak_phase':>10}  "
    ...
    for r in rows:
        ...
        if p is None or f is None:
            ...
            continue
        print(
            f"{p['precision']:<10} {p['vocab']:>10} {p['k']:>7} {p['phase']:>10}  "
            f"{f['vocab']:>11} {f['k']:>7} {f['graduated']!s:>9} {f['steps']:>10} {f['elapsed_s']:>9}"
        )
