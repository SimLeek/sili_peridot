...
def run_config(name, kwargs, seeds, n_steps):
    ...
    return mean, per_seed, skip_rate
...
def main():
    ...
    for name, kwargs in CONFIGS:
        mean, per_seed, skip_rate = run_config(name, kwargs, seeds, args.steps)
        ref = REFERENCE[name]["mean"]
        delta = mean - ref
        flag = "OK" if delta >= -0.05 else "REGRESSED"
        rows.append((name, mean, ref, delta, flag, skip_rate, per_seed))

    print(f"{'config':<18} {'mean':>7} {'ref':>7} {'delta':>7}  {'flag':<10} {'skip_rate':>10}")
    for name, mean, ref, delta, flag, skip_rate, per_seed in rows:
        print(f"{name:<18} {mean:>7.4f} {ref:>7.4f} {delta:>+7.4f}  {flag:<10} {skip_rate:>9.3%}")
