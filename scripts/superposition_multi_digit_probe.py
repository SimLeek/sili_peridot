...
def _make_log_fn(log_f, hidden_width, arm, n_steps):
    ...
    def log_fn(step, total_steps, current, best):
        ...
        log_f.write(f"  hw={hidden_width:>3} {arm:<16} step={step:>7}/{total_steps} "
...
def run_width(log_f, hidden_width, seed=6000):
    ...
    log_f.write(f"--- hidden_width={hidden_width} n_features={n_features} n_steps={n_steps} "
                f"decay={decay:.4f} baseline={baseline:.4f} ---\n")
    width_t0 = time.time()

    md_stoch_encoder = TrueMultiDigitLayer(n_features, hidden_width, max_weights, num_cpus=1,
                                           digit_cls=DISLDOLayer, n_stages=2, base=12.0, lr_power=0.0,
                                           dense=True, rng=np.random.default_rng(seed + 1))
    md_stoch_decoder = TrueMultiDigitLayer(hidden_width, n_features, max_weights, num_cpus=1,
                                           digit_cls=DISLDOLayer, n_stages=2, base=12.0, lr_power=0.0,
                                           dense=True, rng=np.random.default_rng(seed + 2))
    md_stoch_report = measure_superposition(
        md_stoch_encoder, md_stoch_decoder, n_features, hidden_width, DENSITY, n_steps, LR,
        seed=seed + 3, importance_decay=decay,
        log_fn=_make_log_fn(log_f, hidden_width, "multi_digit_stoch", n_steps))

    md_det_encoder = TrueMultiDigitLayer(n_features, hidden_width, max_weights, num_cpus=1,
                                         digit_cls=DISLDOLayerDeterministic, n_stages=2, base=12.0,
                                         lr_power=0.0, dense=True, rng=np.random.default_rng(seed + 4))
    md_det_decoder = TrueMultiDigitLayer(hidden_width, n_features, max_weights, num_cpus=1,
                                         digit_cls=DISLDOLayerDeterministic, n_stages=2, base=12.0,
                                         lr_power=0.0, dense=True, rng=np.random.default_rng(seed + 5))
    md_det_report = measure_superposition(
        md_det_encoder, md_det_decoder, n_features, hidden_width, DENSITY, n_steps, LR,
        seed=seed + 6, importance_decay=decay,
    ...
    fp32_report = measure_superposition(
        fp32_encoder, fp32_decoder, n_features, hidden_width, DENSITY, n_steps, LR,
        seed=seed + 9, importance_decay=decay,
        log_fn=_make_log_fn(log_f, hidden_width, "fp32", n_steps))

    result = {
        "hidden_width": hidden_width, "n_steps": n_steps, "baseline": baseline,
    ...
    log_f.write(f"--- hidden_width={hidden_width} DONE in {time.time() - width_t0:.1f}s: "
