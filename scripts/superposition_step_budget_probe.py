...
if __name__ == "__main__":
    ...
    print(
        f"{'n_steps':>8} {'baseline':>9} {'float_adam':>11} {'fp4_det':>9} {'fp4_stoch':>10} {'fp32':>9} "
        f"{'adam_beats_base':>16}",
        flush=True,
    )
    for n_steps in STEP_BUDGETS:
        r = run_steps(n_steps)
        print(
            f"{r['n_steps']:>8} {r['baseline']:>9.4f} {r['float_adam']:>11.4f} {r['fp4_det']:>9.4f} "
            f"{r['fp4_stoch']:>10.4f} {r['fp32']:>9.4f} {r['float_adam'] < r['baseline']!s:>16}",
            flush=True,
        )
