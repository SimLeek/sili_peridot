...
def _open_log():
    ...
    f.write(
        f"HIDDEN_WIDTH={HIDDEN_WIDTH} baseline={BASELINE:.4f} "
...
def _make_log_fn(log_f, arm, n_steps):
    ...
    def log_fn(step, total_steps, current, best):
        ...
        if crossed and crossed_at["step"] is None:
            ...
            log_f.write(
                f"  {arm:<18} *** CROSSED BASELINE at step={step} current={current:.4f} < baseline={BASELINE:.4f} ***\n"
            )
        ...
        log_f.write(
            f"  {arm:<18} step={step:>7}/{total_steps} current={current:.4f} best={best:.4f} "
...
def run_fp4_stoch(log_f):
    ...
    encoder = DISLDOLayer(
        N_FEATURES, HIDDEN_WIDTH, max_weights, num_cpus=1, rng=np.random.default_rng(seed + 4), dense=True
    )
    decoder = DISLDOLayer(
        HIDDEN_WIDTH, N_FEATURES, max_weights, num_cpus=1, rng=np.random.default_rng(seed + 5), dense=True
    )
...
if __name__ == "__main__":
    ...
    log_f.write(
        f"--- fp4_stoch DONE: best={fp4_report.best_weighted_loss:.4f} "
        f"baseline={BASELINE:.4f} crossed_at={fp4_crossed} ---\n"
    )

    md_report, md_crossed = run_multi_digit_stoch(log_f)
    log_f.write(
        f"--- multi_digit_stoch DONE: best={md_report.best_weighted_loss:.4f} "
