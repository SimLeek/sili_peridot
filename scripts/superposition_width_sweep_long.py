...
def _make_log_fn(log_f, hidden_width: int, arm: str, n_steps: int):
    ...
    def log_fn(step, total_steps, current, best):
        ...
        log_f.write(f"  hw={hidden_width:>3} {arm:<12} step={step:>7}/{total_steps} "
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(header, end="")
        for r in results:
            line = (f"{r['hidden_width']:>9} {r['n_steps']:>8} {r['baseline']:>9.4f} "
                    ...
                    f"{str(r['fp4_stoch'] < r['baseline']):>17}\n")
            log_f.write(line)
            print(line, end="")
        log_f.write(f"=== TOTAL elapsed {time.time() - sweep_t0:.1f}s "
                    f"({(time.time() - sweep_t0) / 60.0:.1f} min) ===\n")
        log_f.close()
