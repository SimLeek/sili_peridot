...
from model.eval_stuck_weights import snapshot_multi_digit_state, check_stuck_weights
...
def run_probe(digit_backend, label):
    ...
    print(f"[{label}] n_synapses={report.n_synapses} "
          ...
          f"mean_delta_w_overall={report.mean_delta_w_overall:.6f}", flush=True)
    return report
