...
| 2026-09-19 | 288 (36) | ~351k | dense fwd + Arm C backward gate (cutoff=0.3, ~40.3% density), peak_lr = 0.01/p **(1/p compensation)** | 0.0248 | vocab=64, k=2 (stuck) | 16,724 (then flat, never reached k=3) | 100,000 steps / 19,417s (~5.39h) full budget | 5.15 | [`8cb949a`](https://github.com/SimLeek/sili_peridot/commit/8cb949a) | `research/dense-vs-sparse-mqar-300k` | `launch_armc_lr_scaled_invp.py` | Weaker than BOTH successful test 3 grid cells (which reached k=3). The 1/p "scale LR up as density drops" formula doesn't hold up at this intermediate density either -- consistent with test 3's own finding that there's no clean monotonic lr(p) trend, just narrow per-density sweet spots |
| 2026-09-19 | 288 (36) | ~351k | dense fwd + Arm C backward gate (cutoff=0.3, ~40.3% density), peak_lr = 0.01/sqrt(p) **(1/sqrt(p), milder bracket)** | 0.0158 | vocab=64, k=3 (stuck) | 9,521 (then flat, never reached vocab=126) | 100,000 steps / 19,228s (~5.34h) full budget | 5.20 | [`8cb949a`](https://github.com/SimLeek/sili_peridot/commit/8cb949a) | `research/dense-vs-sparse-mqar-300k` | `launch_armc_lr_scaled_invsqrtp.py` | Better than 1/p (reached k=3, not just k=2) and FAST early (vocab=64/k=3 by step 9,521 -- comparable to dense's own 8,467) but then hit a hard wall at vocab=64, same shape as unscaled dense's own stall. Milder exponent beat the stronger one, same alpha=1.0-vs-0.5 lesson as the width-scaling law -- neither compensation formula reached the full vocab=126 milestone test 3's grid did at this general density range |
