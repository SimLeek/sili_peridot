...
| 2026-09-18 | 288 (36) | ~351k | Arm C backward + knee-adaptive forward (3 remaining variants: skip-k1, margin=0, margin=0.15) | 0.015 | in progress (arch-sandbox) | TBD | TBD | ~1.0-2.1 (contended, 4-way on 16 threads) | [`72663b9`](https://github.com/SimLeek/sili_peridot/commit/72663b9) | `research/dense-vs-sparse-mqar-300k` | `launch_arm_c_plus_knee_{skip_k1,margin0,margin15}.py` | Predates the LR fix too -- worth rerunning at peak_lr~=0.01 once these finish/are judged stuck |
| 2026-09-18 | 128 (16) | ~70k | dy_r_target + fixed absolute per-step backward count (dy_k_min=dy_k_max=64), dense=True | 0.015 | vocab=16, k=3 (stuck) | 925 (then flat) | 100,000 steps / 9,080s (~2.52h) full budget | 11.01 | [`4c378fc`](https://github.com/SimLeek/sili_peridot/commit/4c378fc) | `research/dense-vs-sparse-mqar-300k` | `launch_dy_fixed_count_width128.py` | **Worst result of the whole investigation**: ONE level-up in 100k steps, flat for the remaining 99,075. Suggests dy_k_min=dy_k_max=64 (forcing exactly half of state_width=128's output columns per sample, overriding dy_r_target=0.9's natural selection) is itself harmful, not just a safe stand-in for the structural cap -- see width=288 companion below before concluding |
| 2026-09-18 | 288 (36) | ~351k | dy_r_target + fixed absolute per-step backward count (dy_k_min=dy_k_max=64), dense=True | 0.015 | in progress (arch-sandbox) | TBD | TBD | TBD | [`4c378fc`](https://github.com/SimLeek/sili_peridot/commit/4c378fc) | `research/dense-vs-sparse-mqar-300k` | `launch_dy_fixed_count_width288.py` | Width=288 companion to the row above -- does this avoid the LR degeneracy the way dense=True+structural cap couldn't safely test |
