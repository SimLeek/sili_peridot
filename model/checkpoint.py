...
def verify_checkpoint_matches_config(state_dict: dict[str, torch.Tensor], cfg: MiniCPM5Config) -> None:
    ...
    if not cfg.tie_word_embeddings:
        assert state_dict["lm_head.weight"].data_ptr() != state_dict["model.embed_tokens.weight"].data_ptr(), (
            "config says tie_word_embeddings=False but lm_head/embed_tokens share storage"
        )

    n_layers_found = len({int(name.split(".")[2]) for name in state_dict if name.startswith("model.layers.")})
    assert n_layers_found == cfg.num_hidden_layers, (
        f"found {n_layers_found} layers in checkpoint, config says {cfg.num_hidden_layers}"
    )
