...
from .sili_block import (
    _ActivationDensity, _density_for_suffix, _forward,
    rmsnorm, rope_cos_sin, apply_rotary, silu,
)
...
def apply_tile_step(
    ...
    q = _forward(tile_layers[".self_attn.q_proj.weight"], normed,
                 _density_for_suffix(activation_density, ".self_attn.q_proj.weight"))
    k = _forward(tile_layers[".self_attn.k_proj.weight"], normed,
                 _density_for_suffix(activation_density, ".self_attn.k_proj.weight"))
    v = _forward(tile_layers[".self_attn.v_proj.weight"], normed,
    ...
    positions = np.clip(
        tick_index - (num_tiles - 1) + np.arange(num_tiles), 0, None)
    ...
    attn_out = _forward(tile_layers[".self_attn.o_proj.weight"], attn_out,
    ...
    gate_mlp = _forward(tile_layers[".mlp.gate_proj.weight"], normed2,
                        _density_for_suffix(activation_density, ".mlp.gate_proj.weight"))
    up_mlp = _forward(tile_layers[".mlp.up_proj.weight"], normed2,
                      _density_for_suffix(activation_density, ".mlp.up_proj.weight"))
    mlp_out = _forward(tile_layers[".mlp.down_proj.weight"], silu(gate_mlp) * up_mlp,
...
def run_tile_recurrence(
    ...
    for i in range(T):
        ...
        M, logits, _aux_loss = apply_tile_step(
            x_window, i, M, tile_layers, input_ln_weight, post_attn_ln_weight,
            cfg, tile_state.centers, tile_state.log_sigmas, energy_dynamics,
            lm_head, num_cpus, activation_density)
