...
from model.toy_precision_models import AdamRank1DISLDOLayer, AdamRowScaleDISLDOLayer, TrueMultiDigitLayer
...
def _model(disldo_cls=DISLDOLayer, num_tiles=NUM_TILES, num_cpus=2):
    return ToyTileRecurrenceRealFP4(
        VOCAB, EMBED_WIDTH, COLUMN_NEURONS, MLP_HIDDEN, num_tiles, MAX_WEIGHTS, num_cpus=num_cpus, disldo_cls=disldo_cls
    )
...
class TestToyTileRecurrenceRealFP4WithRowScaleAdam:
    def test_shapes_and_finite(self):
        ...
        x_window = np.random.RandomState(1).randn(NUM_TILES, EMBED_WIDTH).astype(np.float32) * 0.1
        M_prev = np.zeros((NUM_TILES, STATE_WIDTH), dtype=np.float32)
        M_new, logits, _aux_loss = model.step(x_window, M_prev, learning_rate=0.01)
        assert M_new.shape == (NUM_TILES, STATE_WIDTH)
        assert logits.data.shape == (NUM_TILES, VOCAB)
        assert np.all(np.isfinite(M_new))
        assert np.all(np.isfinite(logits.data))
...
@pytest.mark.integration  # N_STEPS=3000 real training run, not <10s pre-commit territory
class TestToyTileRecurrenceRealFP4LongHorizonStability:
    ...
    def _production_model(self, num_tiles=NUM_TILES, num_cpus=1):
        disldo_cls = functools.partial(TrueMultiDigitLayer, digit_cls=DISLDOLayer, n_stages=3, base=12.0, lr_power=0.0)
        return ToyTileRecurrenceRealFP4(
            VOCAB,
            EMBED_WIDTH,
            COLUMN_NEURONS,
