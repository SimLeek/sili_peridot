...
class TestTrainMqarCurriculumPhase7:
    def test_default_args_run_cleanly_and_report_steps_per_sec(self):
        r = train_curriculum("fp4", max_steps=30, seed=1, peak_lr=0.015,
                             num_tiles=16, k_max=2, log_every=10)
        assert "steps_per_sec" in r
        assert r["steps_per_sec"] > 0
        assert r["total_steps"] >= 1

    def test_widened_sparse_arm_runs_cleanly(self):
        r = train_curriculum("fp4", max_steps=30, seed=2, peak_lr=0.015,
                             num_tiles=16, k_max=2, log_every=10,
                             embed_width=32, input_sparsity_p=0.5,
    ...
    def test_log_fn_receives_steps_per_sec_at_periodic_log_points(self):
        ...
        train_curriculum("fp4", max_steps=25, seed=3, peak_lr=0.015,
                         num_tiles=16, k_max=2, log_every=10, log_fn=log_fn)
