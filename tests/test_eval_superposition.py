...
class TestSampleSparseFeatures:
    def test_density_matches_over_many_samples(self):
        ...
        active_fraction = np.mean(
            [np.mean(sample_sparse_features(rng, n_features, density) != 0.0) for _ in range(200)]
        )
...
class TestNoSuperpositionBaseline:
    ...
    def test_matches_hand_computation(self):
        ...
        # expected = 2.0 * 0.3 / 3.0  # noqa: ERA001 -- explanatory math, not dead code
        assert no_superposition_baseline(imp, hidden_width=1, density=0.3) == pytest.approx(2.0 * 0.3 / 3.0)


@pytest.mark.integration  # n_steps=1500 real training runs
...
class TestSuperpositionRealModel:
    def test_fp4_vs_float32_packing_at_matched_width_and_density(self):
        ...
        fp4_det_encoder = DISLDOLayerDeterministic(
            n_features, hidden_width, n_features * hidden_width, num_cpus=1, rng=np.random.default_rng(2001), dense=True
        )
        fp4_det_decoder = DISLDOLayerDeterministic(
            hidden_width, n_features, hidden_width * n_features, num_cpus=1, rng=np.random.default_rng(2002), dense=True
        )
        fp4_det_report = measure_superposition(
            fp4_det_encoder, fp4_det_decoder, n_features, hidden_width, density, n_steps, lr, seed=2003
        )

        fp4_stoch_encoder = DISLDOLayer(
            n_features, hidden_width, n_features * hidden_width, num_cpus=1, rng=np.random.default_rng(2007), dense=True
        )
        fp4_stoch_decoder = DISLDOLayer(
            hidden_width, n_features, hidden_width * n_features, num_cpus=1, rng=np.random.default_rng(2008), dense=True
        )
        fp4_stoch_report = measure_superposition(
            fp4_stoch_encoder, fp4_stoch_decoder, n_features, hidden_width, density, n_steps, lr, seed=2009
        )

        fp32_encoder = DISLDOLayer32(
            n_features, hidden_width, n_features * hidden_width, num_cpus=1, rng=np.random.default_rng(2004)
        )
        fp32_decoder = DISLDOLayer32(
            hidden_width, n_features, hidden_width * n_features, num_cpus=1, rng=np.random.default_rng(2005)
        )
        fp32_report = measure_superposition(
            fp32_encoder, fp32_decoder, n_features, hidden_width, density, n_steps, lr, seed=2006
        )
